...
import { useRekeningBelanjaMainForm, useRekeningBelanjaTable } from "@/hook/use-rekening-belanja-store";
...
export default function Rekeningbelanja() {  
  ...
  const {clearMainForm} = useRekeningBelanjaMainForm()
    ...
    return (
        <div className="mx-5 mt-5 bg-white p-5 text-accentdarken">
            <div className="flex flex-col xl:flex-row gap-5 justify-between">
                ...
                <button type="submit" className="bg-accent text-white py-3 px-4 rounded-sm hover:bg-accentdarken transition-all" onClick={handleTambahRekening}>Tambah Rekening</button>
            </div>
